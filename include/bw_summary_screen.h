...
// The vendor wrote !IS_AWR here. IS_AWR is not a general symbol -- it is an
// alias the host hack defines in its own include/config/general.h as
// `#define IS_AWR IS_FRLG`, so the real meaning is "not a FireRed/LeafGreen
// build". Substituted rather than stubbed. IS_FRLG is 0 for our Emerald target,
// so this is TRUE and the contest moves page is shown.
// A run has no contests, so FALSE is arguably right here -- left as the vendor
// had it because that is a gameplay call, not a port decision.
#define BW_SUMMARY_SHOW_CONTEST_MOVES               !IS_FRLG               // show the contest moves page
