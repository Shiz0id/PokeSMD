...
// Defined in upstream's src/strings.c, which this port does not take. Kept
// local for the same reason blackjack's eight are. gText_NicknameGacha was
// declared here too and never used -- an extern for a symbol that does not
// exist in this build is silent until someone references it, which is how this
// one got as far as the linker.
...
// BGSetup, BGRed, Shake1 and Shake2 each swap the background image by calling
// InitBgsFromTemplates and then handing GACHA_BG_BASE a brand new
// AllocZeroed(BG_SCREEN_SIZE). Nothing frees the one already there, and
// InitBgFromTemplate has just set sGpuBgConfigs2[bg].tilemap to NULL, so the
// old pointer is gone before anything could free it.
//
// One pull runs BGSetup, Shake1, Shake2, BGSetup, BGRed -- five allocations,
// four of them orphaned, 8 KB of a 113 KB heap per pull. That is a crash in
// well under twenty pulls, and this is the worst of the leaks in the port
// because it compounds WITHIN a session rather than across sessions.
//
// The free has to come before InitBgsFromTemplates, for the reason above.
...
static void GachaMain(u8 taskId)
{
	...
	case STATE_POKEBALL_ARRIVE_WAIT:		
		if (gSprites[sGacha->bouncingPokeballSpriteId].callback == SpriteCallbackDummy)
			{	CreateRandomMon(&mon, sGacha->CalculatedSpecies, level);
				...
				// CreateMonPicSprite_Affine decompresses the pic into its own
				// buffer and loads the palette itself, so upstream's follow-up
				// LoadCompressedSpritePalette, SetMultiuseSpriteTemplateToPokemon
				// and HandleLoadSpecialPokePic_2 are all redundant here. The last
				// one is worse than redundant: it wrote through gMonSpritesGfxPtr,
				// which AllocateMonSpritesGfx only fills for the duration of a
				// battle and which is NULL out on the field.
				//
				// The shiny flag is the mon's own. Upstream passed SHINY_ODDS
				// into this argument, which is non-zero, so every prize was drawn
				// shiny no matter what was actually rolled.
				sGacha->monSpriteId = CreateMonPicSprite_Affine(sGacha->CalculatedSpecies,
															IsMonShiny(&mon),
															GetMonData(&mon, MON_DATA_PERSONALITY),
															MON_PIC_AFFINE_FRONT, 120, 60, 14, TAG_NONE);
