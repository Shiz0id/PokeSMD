...
struct Fanfare
{
    u16 songNum;
    u16 duration;
    // The GBS arrangement of a fanfare is not the same length as the m4a one,
    // so the counter that holds the field locked has to differ too. Rows with
    // no GBS mapping just repeat duration.
    u16 durationGBS;
};
