...
static void CreateBerry(void)
{
    ...
    while (1)
	{
		...
		if (!occupied) {
			sSnake->BerryX = x;
			sSnake->BerryY = y;
			// Take the pixel position from the tile we just picked. Upstream
			// re-scanned SnakeTilesArray afterwards looking for the x/y it had
			// only just read out of it, which left xf/yf provably unassigned to
			// the compiler and cost a MAX_TILES scan to recover what is right
			// here in i.
	...
	switch (sSnake->state)
	{
		...
		case SNAKE_STATE_GAMEOVER:
			if (sSnake->timer == 0)
			{
				...
				VarSet(VAR_GC_FLIP_WINNINGS, (sSnake->Total + sSnake->Total));
				StartExitSnake();
			}
			else
			{
