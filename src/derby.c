...
// .tag was GFX_NAME_RATTATA, copy-pasted from the Rattata sheet below. Harmless
// upstream only because the single call site hand-wrote s.tag =
// GFX_NAME_RAPIDASH and ignored what the struct said. It stops being harmless
// the moment anything loads this sheet through the struct, which is exactly
// what LoadCompressedSpriteSheet does.
...
void GetNewDerby(void)
{
	...
	VarSet(VAR_GC_RACER_1, (num100_1 * 100) + (num10_1 * 10) + num1_1);
	VarSet(VAR_GC_RACER_2, (num100_2 * 100) + (num10_2 * 10) + num1_2);
	VarSet(VAR_GC_RACER_3, (num100_3 * 100) + (num10_3 * 10) + num1_3);
	VarSet(VAR_GC_RACER_4, (num100_4 * 100) + (num10_4 * 10) + num1_4);
	VarSet(VAR_GC_RACER_5, (num100_5 * 100) + (num10_5 * 10) + num1_5);
	VarSet(VAR_GC_RACER_6, (num100_6 * 100) + (num10_6 * 10) + num1_6);
	
	VarSet(VAR_GC_NEW_DERBY, 1);
}
...
static void UpdatePokemonSpeciesNames(void)
{
	...
	else // was: MenuPosition == 5. else so the value is provably assigned
	{
...
static void UpdateNicknames(void)
{
	...
	if (sDerby->MenuPosition == 0)
	{
		if (sDerby->Species_1 < 5) // Ponyta
		{
			for (i = 0; i < MAX_NAMES; i++) {
				if (PonytaNamesValues[i].ID == (VarGet(VAR_GC_RACER_NAME_1))) {
		...
		{
			for (i = 0; i < MAX_NAMES; i++) {
				if (RattataNamesValues[i].ID == (VarGet(VAR_GC_RACER_NAME_1))) {
		...
		{
			for (i = 0; i < MAX_NAMES; i++) {
				if (RapidashNamesValues[i].ID == (VarGet(VAR_GC_RACER_NAME_1))) {
		...
		{
			for (i = 0; i < MAX_NAMES; i++) {
				if (FeebasNamesValues[i].ID == (VarGet(VAR_GC_RACER_NAME_1))) {
	...
	else if (sDerby->MenuPosition == 1)
	{
		...
		{
			for (i = 0; i < MAX_NAMES; i++) {
				if (PonytaNamesValues[i].ID == (VarGet(VAR_GC_RACER_NAME_2))) {
					ShowName(PonytaNamesValues[i].NameString);
				}
			}
		}
		...
		{
			for (i = 0; i < MAX_NAMES; i++) {
				if (RattataNamesValues[i].ID == (VarGet(VAR_GC_RACER_NAME_2))) {
					ShowName(RattataNamesValues[i].NameString);
				}
			}
		}
		...
		{
			for (i = 0; i < MAX_NAMES; i++) {
				if (RapidashNamesValues[i].ID == (VarGet(VAR_GC_RACER_NAME_2))) {
					ShowName(RapidashNamesValues[i].NameString);
				}
			}
		}
		...
		{
			for (i = 0; i < MAX_NAMES; i++) {
				if (FeebasNamesValues[i].ID == (VarGet(VAR_GC_RACER_NAME_2))) {
					ShowName(FeebasNamesValues[i].NameString);
				}
			}
		}
	...
	else if (sDerby->MenuPosition == 2)
	{
		...
		{
			for (i = 0; i < MAX_NAMES; i++) {
				if (PonytaNamesValues[i].ID == (VarGet(VAR_GC_RACER_NAME_3))) {
					ShowName(PonytaNamesValues[i].NameString);
				}
			}
		}
		...
		{
			for (i = 0; i < MAX_NAMES; i++) {
				if (RattataNamesValues[i].ID == (VarGet(VAR_GC_RACER_NAME_3))) {
					ShowName(RattataNamesValues[i].NameString);
				}
			}
		}
		...
		{
			for (i = 0; i < MAX_NAMES; i++) {
				if (RapidashNamesValues[i].ID == (VarGet(VAR_GC_RACER_NAME_3))) {
					ShowName(RapidashNamesValues[i].NameString);
				}
			}
		}
		...
		{
			for (i = 0; i < MAX_NAMES; i++) {
				if (FeebasNamesValues[i].ID == (VarGet(VAR_GC_RACER_NAME_3))) {
					ShowName(FeebasNamesValues[i].NameString);
				}
			}
		}
	...
	else if (sDerby->MenuPosition == 3)
	{
		...
		{
			for (i = 0; i < MAX_NAMES; i++) {
				if (PonytaNamesValues[i].ID == (VarGet(VAR_GC_RACER_NAME_4))) {
					ShowName(PonytaNamesValues[i].NameString);
				}
			}
		}
		...
		{
			for (i = 0; i < MAX_NAMES; i++) {
				if (RattataNamesValues[i].ID == (VarGet(VAR_GC_RACER_NAME_4))) {
					ShowName(RattataNamesValues[i].NameString);
				}
			}
		}
		...
		{
			for (i = 0; i < MAX_NAMES; i++) {
				if (RapidashNamesValues[i].ID == (VarGet(VAR_GC_RACER_NAME_4))) {
					ShowName(RapidashNamesValues[i].NameString);
				}
			}
		}
		...
		{
			for (i = 0; i < MAX_NAMES; i++) {
				if (FeebasNamesValues[i].ID == (VarGet(VAR_GC_RACER_NAME_4))) {
					ShowName(FeebasNamesValues[i].NameString);
				}
			}
		}
	...
	else if (sDerby->MenuPosition == 4)
	{
		...
		{
			for (i = 0; i < MAX_NAMES; i++) {
				if (PonytaNamesValues[i].ID == (VarGet(VAR_GC_RACER_NAME_5))) {
					ShowName(PonytaNamesValues[i].NameString);
				}
			}
		}
		...
		{
			for (i = 0; i < MAX_NAMES; i++) {
				if (RattataNamesValues[i].ID == (VarGet(VAR_GC_RACER_NAME_5))) {
					ShowName(RattataNamesValues[i].NameString);
				}
			}
		}
		...
		{
			for (i = 0; i < MAX_NAMES; i++) {
				if (RapidashNamesValues[i].ID == (VarGet(VAR_GC_RACER_NAME_5))) {
					ShowName(RapidashNamesValues[i].NameString);
				}
			}
		}
		...
		{
			for (i = 0; i < MAX_NAMES; i++) {
				if (FeebasNamesValues[i].ID == (VarGet(VAR_GC_RACER_NAME_5))) {
					ShowName(FeebasNamesValues[i].NameString);
				}
			}
		}
	...
	else if (sDerby->MenuPosition == 5)
	{
		...
		{
			for (i = 0; i < MAX_NAMES; i++) {
				if (PonytaNamesValues[i].ID == (VarGet(VAR_GC_RACER_NAME_6))) {
					ShowName(PonytaNamesValues[i].NameString);
				}
			}
		}
		...
		{
			for (i = 0; i < MAX_NAMES; i++) {
				if (RattataNamesValues[i].ID == (VarGet(VAR_GC_RACER_NAME_6))) {
					ShowName(RattataNamesValues[i].NameString);
				}
			}
		}
		...
		{
			for (i = 0; i < MAX_NAMES; i++) {
				if (RapidashNamesValues[i].ID == (VarGet(VAR_GC_RACER_NAME_6))) {
					ShowName(RapidashNamesValues[i].NameString);
				}
			}
		}
		...
		{
			for (i = 0; i < MAX_NAMES; i++) {
				if (FeebasNamesValues[i].ID == (VarGet(VAR_GC_RACER_NAME_6))) {
					ShowName(FeebasNamesValues[i].NameString);
				}
			}
		}
...
static void SetNicknames(void)
{
    ...
    VarSet(VAR_GC_RACER_NAME_1, name1);
    VarSet(VAR_GC_RACER_NAME_2, name2);
    VarSet(VAR_GC_RACER_NAME_3, name3);
    VarSet(VAR_GC_RACER_NAME_4, name4);
    VarSet(VAR_GC_RACER_NAME_5, name5);
    VarSet(VAR_GC_RACER_NAME_6, name6);
	
	VarSet(VAR_GC_NICKNAME_TOGGLE, 1);
}
...
static void UpdatePokemonData(void)
{
	...
	UpdatePayout();
	UpdateUI();
	if (VarGet(VAR_GC_NICKNAME_TOGGLE) == 0)
	{
		SetNicknames();
	}
	UpdateNicknames();
...
{
	// Called every time the bet menu flips between its two pages, and both
	// branches allocate a fresh tilemap after InitBgsFromTemplates has nulled
	// the pointer to the old one. Same bug as the gacha's shake and pachinko's
	// LevelChange; the free has to come before InitBgsFromTemplates.
	GameCorner_FreeBgTilemapBuffers();
...
{
	// The chain below covers RacerSelected 1..6 with no else, and unlike the
	// MenuPosition chains it is 1-based -- so a RacerSelected of 0 fell through
	// and drew the cursor at a stack-garbage y and subpriority. Default to the
	// racer 1 row, which is where a cursor with nothing selected belongs.
	u8 y = 40;
	u8 position = 10;

	LoadSpritePalettes(sSpritePalettes);

	if (sDerby->RacerSelected == 2)
	{
		position = 8;
		y = 40 + 16;
	}
...
static void InitRacetrack(void)
{
	...
	// Swapping the bet menu for the racetrack; free what the menu was using.
	GameCorner_FreeBgTilemapBuffers();
	InitBgsFromTemplates(0, sDerbyBGtemplates, ARRAY_COUNT(sDerbyBGtemplates));
	SetBgTilemapBuffer(DERBY_BG_BASE, AllocZeroed(BG_SCREEN_SIZE));
		...
		if ((gSprites[sDerby->RacerSprite1].x > 16) &&
			...
			else
			{
				...
				if (sDerby->RacerSelected == 1)
				{
					...
					{
						VarSet(VAR_GC_FLIP_WINNINGS, sDerby->PotentialWin);
					}
					else if (sDerby->place1 == 1) // Second
					{
						VarSet(VAR_GC_FLIP_WINNINGS, 0); // Lose
					}
					else
					{
						VarSet(VAR_GC_FLIP_WINNINGS, 0);
					}
				}
				else if (sDerby->RacerSelected == 2)
				{
				...
				VarSet(VAR_GC_NEW_DERBY, 0);
				VarSet(VAR_GC_NICKNAME_TOGGLE, 0);
				sDerby->delay = 120;
