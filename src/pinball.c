...
static void HandleBallPhysics(void)
{
    ...
    if ((ball->yPos >> 8) > 168)
    {
        // Was `==`. The clamp never happened, so the ball kept whatever
        // below-the-table position it had drifted to on the frame it was lost
        // rather than being parked at 170. LoseBall changes sPinballGame->state
        // immediately, so this is a cosmetic difference on the losing frame
        // rather than a runaway -- but the statement plainly meant to assign.
        ball->yPos = 170 << 8;
        LoseBall();
    }
}
...
static u8 GetCollisionAttribute(u8 gameType, bool32 ballIsEntering, int index)
{
    ...
    case GAME_TYPE_SEEL:
        ...
        break;
    // Gengar is the default rather than its own case so that both pointers are
    // provably assigned. Upstream listed all four types and no default, which
    // leaves the compiler unable to rule out reading an uninitialised pointer.
    case GAME_TYPE_GENGAR:
    default:
...
static void ExitPinballGame(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        ...
        // Two BG tilemaps at BG_SCREEN_SIZE * 4 -- 8 KB each, 16 KB a session,
        // the largest of these in the port -- plus the window buffers. None of
        // it was freed.
...
static bool32 UpdateDiglett(struct Diglett *diglett)
{
    ...
    else
    {
        ...
        case DUGTRIO_STATE_0ALIVE:
            if (dugtrioSprite->animEnded)
				VarSet(VAR_GC_FLIP_WINNINGS, 150);
				SetPlayerDigits(VarGet(VAR_GC_FLIP_WINNINGS));
...
static void UpdateGhost(struct Gengar *gengar, struct GraveyardGhost *ghost, u8 *numGhostHits, u8 nextState, int numGhosts)
{
	// Gastly is the base rate; Haunter pays more. Upstream tested for both and
	// had no else, so multiplier was only ever assigned because the two call
	// sites happen to pass one of those two counters -- correct by accident,
	// and an uninitialised coin payout the moment a third ghost tier is added.
