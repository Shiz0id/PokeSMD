...
// Rewrites TRAINER_BATTLE_PARAM.mode to match the player's battle mode option.
//
// Split out of BattleSetup_ConfigureTrainerBattle and exported, because the
// roguelike's generated dungeon trainers never reach that function: their
// scripts carry no inline trainerbattle data, so ConfigureTrainerBattle and
// ConfigureTwoTrainersBattle branch around it and fill TRAINER_BATTLE_PARAM
// from RogueDungeon_SetUpTrainerBattle instead. Left where upstream had it,
// the setting would have applied to bosses and script trainers while the
// generated trainers -- most of the battles in a run -- silently ignored it.
