...
// EWRAM_DATA is section(".sbss"), so it cannot carry a non-zero initialiser --
// upstream's `= 1` does not compile here. Set it in InitBJScreen instead, which
// is better anyway: a load-time initialiser only ever held until the first
// ShowMessage overwrote it, so a second session inherited the previous one's
// window id.
static EWRAM_DATA u8 sTextWindowId = 0;

// Upstream puts these eight in src/strings.c, which is an engine file nothing
// else in this port touches. Nothing outside this game reads them, so they stay
// here and strings.c stays clean.
//
// Upstream's two bet lines read "¥{STR_VAR_1}". The glyph exists in our charmap,
// so it would have built -- but this game never touches money. It is GetCoins,
// AddCoins, RemoveCoins and SetCoins throughout, so the bet is in Game Corner
// coins and the money symbol is simply the wrong unit in front of it.
...
{
	gSprites[sBlackJack->option1SpriteId].oam.tileNum = (VarGet(VAR_GC_OPTION1_TILENUM));
	gSprites[sBlackJack->option2SpriteId].oam.tileNum = (VarGet(VAR_GC_OPTION2_TILENUM));
	gSprites[sBlackJack->option3SpriteId].oam.tileNum = (VarGet(VAR_GC_OPTION3_TILENUM));
	...
	{
		gSprites[sBlackJack->option1SpriteId].oam.tileNum = (VarGet(VAR_GC_OPTION1_TILENUM) + 8);
		gSprites[sBlackJack->option2SpriteId].oam.tileNum = (VarGet(VAR_GC_OPTION2_TILENUM) + 8);
		gSprites[sBlackJack->cursorSpriteId].y = 57;
		gSprites[sBlackJack->cursorSpriteId].invisible = FALSE;	
		gSprites[sBlackJack->option1SpriteId].invisible = FALSE;
		gSprites[sBlackJack->option2SpriteId].invisible = FALSE;
	...
	if (num == 2) // HIT, STAND, DOUBLE
	{
		gSprites[sBlackJack->option1SpriteId].oam.tileNum = (VarGet(VAR_GC_OPTION1_TILENUM) + 8);
		gSprites[sBlackJack->option2SpriteId].oam.tileNum = (VarGet(VAR_GC_OPTION2_TILENUM) + 8);
		gSprites[sBlackJack->option3SpriteId].oam.tileNum = (VarGet(VAR_GC_OPTION3_TILENUM) + 8);
		gSprites[sBlackJack->cursorSpriteId].y = 57;
		gSprites[sBlackJack->cursorSpriteId].invisible = FALSE;
		gSprites[sBlackJack->option1SpriteId].invisible = FALSE;
		gSprites[sBlackJack->option2SpriteId].invisible = FALSE;
	...
	if (num == 3) // HIT, STAND, INSURE
	{
		gSprites[sBlackJack->option1SpriteId].oam.tileNum = (VarGet(VAR_GC_OPTION1_TILENUM) + 8);
		gSprites[sBlackJack->option2SpriteId].oam.tileNum = (VarGet(VAR_GC_OPTION2_TILENUM) + 8);
		gSprites[sBlackJack->option3SpriteId].oam.tileNum = (VarGet(VAR_GC_OPTION3_TILENUM) + 16);
		gSprites[sBlackJack->cursorSpriteId].y = 57;
		gSprites[sBlackJack->cursorSpriteId].invisible = FALSE;
		gSprites[sBlackJack->option1SpriteId].invisible = FALSE;
		gSprites[sBlackJack->option2SpriteId].invisible = FALSE;
...
{	
	...
	FlagClear(FLAG_GC_INSURANCE);
	VarSet(VAR_GC_PLAYER_BJ, 0);
	VarSet(VAR_GC_DEALER_BJ, 0);
	
	if ((GetCoins()) >= 10) // If you can afford to play
		{
		VarSet(VAR_GC_OPTIONS_BJ, 0); // 0 = Bet, 1 = Hit, 2 = Double, 3 = Insurance, 4 = None
		}
	else // Not enough coins
		{
		VarSet(VAR_GC_OPTIONS_BJ, 4); // 0 = Bet, 1 = Hit, 2 = Double, 3 = Insurance, 4 = None
		}
	
	FlagClear(FLAG_GC_BLACKJACK);
	CreateCreditSprites();
	
	CreatePlayerSprites();
	CreateDealerSprites();
	SetCreditDigits(GetCoins());
	SetPlayerDigits(VarGet(VAR_GC_PLAYER_BJ));
	SetDealerDigits(VarGet(VAR_GC_DEALER_BJ));
	CreateCursorSprite();
	CreateOptions();
	VarSet(VAR_GC_OPTION1_TILENUM, (gSprites[sBlackJack->option1SpriteId].oam.tileNum));
	VarSet(VAR_GC_OPTION2_TILENUM, (gSprites[sBlackJack->option2SpriteId].oam.tileNum));
	VarSet(VAR_GC_OPTION3_TILENUM, (gSprites[sBlackJack->option3SpriteId].oam.tileNum));
	SetMode(VarGet(VAR_GC_OPTIONS_BJ));
	CreatePopUpSprite();
...
static void HandleInput(void)
{
    ...
    {
        AButton();
		if (FlagGet(FLAG_GC_RESET_BJ) == TRUE) {
			FlagClear(FLAG_GC_RESET_BJ); }
    }
...
static void Reset(void)
{
	...
	ShuffleCards();
	AdjustCards();
	VarSet(VAR_GC_OPTIONS_BJ, 0);
	SetMode(VarGet(VAR_GC_OPTIONS_BJ));
	sBlackJack->exitToggle = 0;
	FlagSet(FLAG_GC_RESET_BJ);
	DeactivateAllTextPrinters();
	PrintInitMessage();
	return;
}
...
static void Win(void)
{
	...
	switch (sBlackJack->winState)
	{
		...
		case WIN_2:
			...
			PlaySE(SE_SHOP);
			if (((VarGet(VAR_GC_BET_BJ) * 2) + GetCoins()) < 9999)
			{
				if (FlagGet(FLAG_GC_BLACKJACK) == TRUE)
				{
					// 3:2 payout
					winnings = (VarGet(VAR_GC_BET_BJ) * 3) / 2;
				...
				{
					// Regular 1:1 payout
					winnings = VarGet(VAR_GC_BET_BJ) * 2;
...
static void Stand(void)
{
	...
	switch (sBlackJack->standState)
	{
		case STAND_0:
			...
			VarSet(VAR_GC_DEALER_BJ, (VarGet(VAR_GC_DEALER_BJ) + DCardPoints));
			SetDealerDigits(VarGet(VAR_GC_DEALER_BJ));
			sBlackJack->waitTimer = 100000;
		...
		case STAND_1:
            ...
            if (sBlackJack->waitTimer > 0) {
				...
				if (FlagGet(FLAG_GC_BLACKJACK) == FALSE) {
		...
		case STAND_2:
			...
			if (FlagGet(FLAG_GC_INSURANCE) == TRUE)
			{
				if (VarGet(VAR_GC_DEALER_BJ) == 21)
				{
					AddCoins(VarGet(VAR_GC_BET_BJ));
					FlagClear(FLAG_GC_INSURANCE);
				}
		...
		case STAND_3:	
			...
			if (VarGet(VAR_GC_DEALER_BJ) < 17) {
				...
				if (DCardPoints == 0)
				{
					if ((VarGet(VAR_GC_DEALER_BJ) + 11) > 21)
					{
						DCardPoints = 1;
					}
					else
					...
					if (VarGet(VAR_GC_DEALER_BJ) > 21)
					{
						if (cardPoints[sBlackJack->dealerCardNumbers[0]] == 0) {
							if (sBlackJack->AceAdjustDealer == 0) {
								VarSet(VAR_GC_DEALER_BJ, (VarGet(VAR_GC_DEALER_BJ) - 10));
		...
		case STAND_5:
			...
			if (VarGet(VAR_GC_DEALER_BJ) < 22) 
			{
				if (VarGet(VAR_GC_DEALER_BJ) > VarGet(VAR_GC_PLAYER_BJ)) // Dealer Wins
				{
					ProcessLose();
					if (FlagGet(FLAG_GC_RESET_BJ) == TRUE) {
					return; }
				}
				else if (VarGet(VAR_GC_DEALER_BJ) < VarGet(VAR_GC_PLAYER_BJ)) // Player Wins
				{
					ProcessWin();
					if (FlagGet(FLAG_GC_RESET_BJ) == TRUE) {
					return; }
				}
				if (VarGet(VAR_GC_DEALER_BJ) == VarGet(VAR_GC_PLAYER_BJ)) // Push
				{
					ProcessPush();
					if (FlagGet(FLAG_GC_RESET_BJ) == TRUE) {
					return; }
			...
			else if (VarGet(VAR_GC_DEALER_BJ) > 21) // Dealer Busts
			{
				ProcessWin();
				if (FlagGet(FLAG_GC_RESET_BJ) == TRUE) {
					return; }
...
static void Double(void)
{
	...
	switch (sBlackJack->doubleState)
	{
		case DOUBLE_0:
			...
			PlaySE(SE_SHOP);
			SetCoins(GetCoins() - VarGet(VAR_GC_BET_BJ));
			VarSet(VAR_GC_BET_BJ, (VarGet(VAR_GC_BET_BJ) * 2));
			SetCreditDigits(GetCoins());
			VarSet(VAR_GC_OPTIONS_BJ, 4);
			SetMode(VarGet(VAR_GC_OPTIONS_BJ));
			sBlackJack->waitTimer = 100000;
		...
		case DOUBLE_2:
			...
			if (playerCardPoints == 0){
                if ((VarGet(VAR_GC_PLAYER_BJ) + 11) > 21)
                {
                    playerCardPoints = 1;
                }
                else
			...
			VarSet(VAR_GC_PLAYER_BJ, (VarGet(VAR_GC_PLAYER_BJ) + playerCardPoints));
			SetPlayerDigits(VarGet(VAR_GC_PLAYER_BJ));
			sBlackJack->waitTimer = 100000;
...
static void Hit(void)
{
    ...
    switch (sBlackJack->hitState)
    {
            ...
            if (VarGet(VAR_GC_PLAYER_BJ) > 21)
			{
				for (i = 0; i < sBlackJack->numPlayerCards; i++) 
				{
					if (cardPoints[sBlackJack->playerCardNumbers[i]] == 0) 
					{
						...
						{
							VarSet(VAR_GC_PLAYER_BJ, (VarGet(VAR_GC_PLAYER_BJ) - 10));
        ...
        case HIT_2:
            ...
            else if (VarGet(VAR_GC_PLAYER_BJ) == 21)
            {
                ProcessStand();
                if (FlagGet(FLAG_GC_RESET_BJ) == TRUE) {
                    return;
...
static void AButton(void)
{
	...
	if (VarGet(VAR_GC_OPTIONS_BJ) != 4)
	{
		if (gSprites[sBlackJack->cursorSpriteId].y == 57) // Top
		{
			if (VarGet(VAR_GC_OPTIONS_BJ) == 0) // +100
			{
				if ((GetCoins()) > 9) // enough
				{
					...
					PlaySE(SE_VEND);
					RemoveCoins(10);
					VarSet(VAR_GC_BET_BJ, (VarGet(VAR_GC_BET_BJ) + 10));
					bet = VarGet(VAR_GC_BET_BJ);
					num = CountDigits(VarGet(VAR_GC_BET_BJ));
					SetCreditDigits(GetCoins());
			...
			if ((VarGet(VAR_GC_OPTIONS_BJ) == 1) || (VarGet(VAR_GC_OPTIONS_BJ) == 2) || (VarGet(VAR_GC_OPTIONS_BJ) == 3)) { // Hit
				if ((VarGet(VAR_GC_BET_BJ)) > 9) // Enough
				{
					PlaySE(SE_CARD);
					//delay?
		...
		if (gSprites[sBlackJack->cursorSpriteId].y == (57 + 16)) // Middle
		{
			if (VarGet(VAR_GC_OPTIONS_BJ) == 0) // -100
			{
				if (VarGet(VAR_GC_BET_BJ) > 0) // Existing Bet
				{
					BGrefresh();
					PlaySE(SE_VEND);
					AddCoins(10);
			...
			if ((VarGet(VAR_GC_OPTIONS_BJ) == 1) || (VarGet(VAR_GC_OPTIONS_BJ) == 2) || (VarGet(VAR_GC_OPTIONS_BJ) == 3)) { // Stand
				PlaySE(SE_CARD);
				// delay?
		...
		{
			if (VarGet(VAR_GC_OPTIONS_BJ) == 0) // Bet
			{
				if ((VarGet(VAR_GC_BET_BJ)) > 9) // Enough
					{
						...
						{
							VarSet(VAR_GC_PLAYER_BJ, playerCard1Points + playerCard2Points);
						}
						else if (playerCard1Points == 0 && playerCard2Points == 0)
						{
							VarSet(VAR_GC_PLAYER_BJ, 12);
						}
						else if (playerCard1Points == 0 && playerCard2Points != 0)
						{
							VarSet(VAR_GC_PLAYER_BJ, 11 + playerCard2Points);
						}
						else if (playerCard1Points != 0 && playerCard2Points == 0)
						{
							VarSet(VAR_GC_PLAYER_BJ, playerCard1Points + 11);
						}
						...
						{
							VarSet(VAR_GC_DEALER_BJ, 11);
						}
						else
						{
							VarSet(VAR_GC_DEALER_BJ, dealerCard1Points);
						}
						SetPlayerDigits(VarGet(VAR_GC_PLAYER_BJ));
						SetDealerDigits(VarGet(VAR_GC_DEALER_BJ));
						if (VarGet(VAR_GC_PLAYER_BJ) == 21)
						...
						if ((VarGet(VAR_GC_PLAYER_BJ) != 9) && (VarGet(VAR_GC_PLAYER_BJ) != 10) && (VarGet(VAR_GC_PLAYER_BJ) != 11) &&
							(VarGet(VAR_GC_DEALER_BJ) != 11))
						{
							VarSet(VAR_GC_OPTIONS_BJ, 1);
							SetMode(VarGet(VAR_GC_OPTIONS_BJ));
							return;
						...
						if ((VarGet(VAR_GC_PLAYER_BJ) >= 9) && (VarGet(VAR_GC_PLAYER_BJ) <= 11) &&
							(VarGet(VAR_GC_DEALER_BJ) != 11))
						{
							if (GetCoins() > VarGet(VAR_GC_BET_BJ)) {
							VarSet(VAR_GC_OPTIONS_BJ, 2);
							SetMode(VarGet(VAR_GC_OPTIONS_BJ));
							}
							else
							{
						...
						if ((VarGet(VAR_GC_DEALER_BJ) == 11))
						{
							if (GetCoins() > (VarGet(VAR_GC_BET_BJ) / 2)) {
							VarSet(VAR_GC_OPTIONS_BJ, 3);
							SetMode(VarGet(VAR_GC_OPTIONS_BJ));
							}
							else
							{
			...
			if (VarGet(VAR_GC_OPTIONS_BJ) == 2) // Double	
			{
				PlaySE(SE_SHOP);
				VarSet(VAR_GC_OPTIONS_BJ, 4);
				SetMode(VarGet(VAR_GC_OPTIONS_BJ));
			...
			if (VarGet(VAR_GC_OPTIONS_BJ) == 3) // Insurance
			{
				PlaySE(SE_SHOP);
				VarSet(VAR_GC_OPTIONS_BJ, 4);
				SetMode(VarGet(VAR_GC_OPTIONS_BJ));
				RemoveCoins(VarGet(VAR_GC_BET_BJ) / 2);
				SetCreditDigits(GetCoins());
				// delay?
				FlagSet(FLAG_GC_INSURANCE);
				VarSet(VAR_GC_OPTIONS_BJ, 1);
				SetMode(VarGet(VAR_GC_OPTIONS_BJ));
			}
			return;
...
static void MoveCursor(int direction)
{
	...
	if (((VarGet(VAR_GC_OPTIONS_BJ)) != 1) && (VarGet(VAR_GC_OPTIONS_BJ) != 4)) {
		switch (direction)
		{
...
static void ExitBJ(void)
{
    if (!gPaletteFade.active)
    {
        ...
        // allocates: two templates at 30x2 and 14x4 tiles, ~3.7 KB, plus
        // whatever ShowMessage's AddWindow left outstanding. Of the five games
        // ported so far this is the only one that opens windows without
        // closing them -- Voltorb Flip is balanced, and the other three use no
        // windows at all.
