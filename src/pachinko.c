...
static void RandomLevel(void)
{
	...
	// Each of the 23 branches below Allocs a fresh collision map (704 bytes for
	// level 1) into the same pointer, and only the exit path ever freed one --
	// so every level after the first orphaned its predecessor. Free here rather
	// than in 23 places.
	TRY_FREE_AND_SET_NULL(diglett->collisionMap);
...
static void LevelChange(void)
{
	...
	// Every level change allocated a fresh BG tilemap and abandoned the last
	// one -- InitBgsFromTemplates nulls sGpuBgConfigs2[bg].tilemap, so the
	// pointer is gone before anything could free it. 2 KB a level, and a
	// session moves through as many levels as the player survives. Same bug as
	// the gacha's shake animation; the free must come first for the same
	// reason. RandomLevel below leaks its collision map on the same schedule.
...
static void DrawMeowthScoreJewels(struct Meowth *meowth)
{
	...
	VarSet(VAR_GC_FLIP_WINNINGS, (meowth->score * 3));
	SetPlayerDigits(VarGet(VAR_GC_FLIP_WINNINGS));
    CopyBgTilemapBufferToVram(PINBALL_BG_BASE);
}
...
static void DrawSeelScoreJewels(struct Seel *seel)
{
	...
	VarSet(VAR_GC_FLIP_WINNINGS, (seel->score * 4));
	SetPlayerDigits(VarGet(VAR_GC_FLIP_WINNINGS));
    CopyBgTilemapBufferToVram(PINBALL_BG_BASE);
}
...
static void HandleBallPhysics(void)
{
    // Pachinko has no flippers and no bumper objects, so the two calls that
    // would set these are commented out below. Upstream left isFlipperColliding
    // uninitialised anyway and then read it, along with the normal it selects,
    // so the collision response was gated on stack garbage. Both are FALSE by
    // construction here, which makes staticCollisionNormal the only one that
    // can be chosen -- the behaviour the commented-out calls imply.
...
static bool32 HandleFlippers(struct Ball *ball, u16 *outYForce, u8 *outCollisionNormal, int *outCollisionAmplification)
{
    ...
    // Pachinko has no flippers, so the whole body above is commented out
    // upstream and this fell off the end of a bool32 function. Nothing calls it
    // -- the one call site is commented out too -- but say so explicitly rather
    // than returning whatever was in r0.
    return FALSE;
}
...
static bool32 CheckFlipperCollision(struct Ball *ball, struct Flipper *flipper, u16 *outYForce, u8 *outCollisionNormal, int *outCollisionAmplification)
{
    ...
    // Body commented out with the flippers; see HandleFlippers above.
    return FALSE;
}
...
static u8 GetCollisionAttribute(u8 gameType, bool32 ballIsEntering, int index)
{
    ...
    case GAME_TYPE_SEEL:
        ...
        break;
    // Default so both pointers are provably assigned; see the same function in
    // pinball.c, which this file is a copy of.
    case GAME_TYPE_GENGAR:
    default:
...
static u8 GetCollisionMaskRow(u8 gameType, int collisionAttribute, int row)
{
    ...
    // Collision attributes 0xE0-0xFF are flipper masks, and the lookup that
    // filled this is commented out below because pachinko has no flippers --
    // leaving `return ReverseBits(mask)` reading an uninitialised local. Zero
    // is the right answer for a table with no flippers in it: no collision.
    u8 mask = 0;
    ...
    if (collisionAttribute < 0xE0)
    {
        ...
        case GAME_TYPE_SEEL:
            ...
            break;
        // Default for the same reason as GetCollisionAttribute above.
        case GAME_TYPE_GENGAR:
        default:
...
static void ExitPinballGame(void)
{
    if (!gPaletteFade.active)
    {
        ...
        if (sPinballGame->gameType == GAME_TYPE_DIGLETT)
            TRY_FREE_AND_SET_NULL(sPinballGame->diglett.collisionMap);
...
static bool32 UpdateDiglett(struct Diglett *diglett)
{
   ...
   //             }
	//			VarSet(VAR_GC_FLIP_WINNINGS, (diglett->numDiglettsHit * 3));
	//			SetPlayerDigits(VarGet(VAR_GC_FLIP_WINNINGS));
   //             break;
   //         }
   //
   ...
   //     //struct Sprite *dugtrioSprite = &gSprites[diglett->dugtrioSpriteId];
	//	//VarSet(VAR_GC_FLIP_WINNINGS, 100);
	//	//SetPlayerDigits(VarGet(VAR_GC_FLIP_WINNINGS));
   ...
   //     //    break;
    ...
    // Body commented out with the Diglett stage; see HandleFlippers above.
    return FALSE;
}   //
...
static void UpdateDugtrioSprite(struct Sprite *sprite)
{
	...
	//		PlayBGM(MUS_NONE);
	//		VarSet(VAR_GC_FLIP_WINNINGS, 200);
	//		SetPlayerDigits(VarGet(VAR_GC_FLIP_WINNINGS));
...
static void UpdateGhost(struct Gengar *gengar, struct GraveyardGhost *ghost, u8 *numGhostHits, u8 nextState, int numGhosts)
{
	// Same as pinball.c, which this file is a copy of: Gastly is the base rate,
	// Haunter pays more, and upstream's two-way test with no else left this
	// assigned only because the call sites happen to pass one of those two.
	u8 multiplier = 3;  // Gastly
    struct Sprite *sprite = &gSprites[ghost->spriteId];

    if (numGhostHits == &gengar->numHaunterHits)
    ...
    case GHOST_STATE_HIT:
        ...
        {
            (*numGhostHits)++;
			VarSet(VAR_GC_FLIP_WINNINGS, (VarGet(VAR_GC_FLIP_WINNINGS) + multiplier));
			SetPlayerDigits(VarGet(VAR_GC_FLIP_WINNINGS));
			PlaySE(SE_M_TELEPORT);
...
static void UpdateGengarSprite(struct Sprite *sprite)
{
    ...
    if (prevState != curState)
    {
        ...
        case GENGAR_STATE_LEAVING:
            StartSpriteAnim(sprite, 4);
			PlayBGM(MUS_NONE);
			VarSet(VAR_GC_FLIP_WINNINGS, 250);
			SetPlayerDigits(VarGet(VAR_GC_FLIP_WINNINGS));
			PlaySE(SE_SUPER_EFFECTIVE);
            break;
        }
