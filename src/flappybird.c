	...
	switch (sFlappy->state)
	{
		...
		case FLAPPY_INPUT:
			...
			if ((sFlappy->bg2ScrollX == 225) || ((sFlappy->bg2ScrollX == 98) && (sFlappy->Obstacle1Id != 0))) // Point
			{
				...
				if (VarGet(VAR_GC_FLIP_WINNINGS) < 9998) {
					VarSet(VAR_GC_FLIP_WINNINGS, VarGet(VAR_GC_FLIP_WINNINGS) + 2); }
				else
				{
					VarSet(VAR_GC_FLIP_WINNINGS, 9999);
				}
		...
		case FLAPPY_GAMEOVER:
			...
			if (sFlappy->timerDelay == 0)
			{
				if (sFlappy->Points > VarGet(VAR_GC_FLAPPY_HISCORE))
				{
					PlayFanfare(MUS_LEVEL_UP);
					VarSet(VAR_GC_FLAPPY_HISCORE, sFlappy->Points);
...
static void InitFlappyBirdScreen(void)
{
	...
	SetScoreDigits(VarGet(VAR_GC_FLAPPY_HISCORE));
	CreateScore();
	CreateScore2();
	CreateButterfree();
	CreateTrail();
