...
void CB2_ShowVoltorbFlip(void)
{
	...
	CreateCreditSprites();
	SetCreditDigits(VarGet(VAR_GC_FLIP_WINNINGS));
	//SetCreditDigits(9876);
	sVflip->prevWinnings = VarGet(VAR_GC_FLIP_WINNINGS);
    ...
    ResetVoltorbFlipCards(VarGet(VAR_GC_FLIP_LEVEL));
...
static void Task_VoltorbFlipWaitForKeyPress(u8 taskId)
{
    ...
    else if(gameState == GAME_STATE_WIN)
    {
		...
		VarSet(VAR_GC_FLIP_LEVEL, VarGet(VAR_GC_FLIP_LEVEL) + 1);
...
static void Task_VoltorbFlipVictory(u8 taskId)
{
    ...
    else
    {
        ...
        {
			VarSet(VAR_GC_FLIP_WINNINGS, sVflip->curWinnings);
            BeginNormalPaletteFade(PALETTES_ALL, 0, 0, 16, RGB_BLACK);
            gTasks[taskId].func = Task_VoltorbFlipFadeOut;
        }
    }
