        ...
        case FISHTASK_OBTAIN_ITEM:
            ...
            if (!IsTextPrinterActiveOnWindow(0) && (JOY_NEW(A_BUTTON) || JOY_NEW(B_BUTTON)))
            {
                if (RoomForItem)
                {
                    ...
                    // The pocket names stopped being global strings; the bag now
                    // carries its own COMPOUND_STRING table. Spelled out here for
                    // the same reason, rather than reaching into the bag's static.
