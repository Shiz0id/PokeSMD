@ HQ-Mixer rev 4.0 by ipatix -- https://github.com/ipatix/gba-hq-mixer
@ Vendored under the MIT License (see LICENSE-hq-mixer). NOTE: this file's own
@ upstream header claims GPLv3 and points at a LICENSE.txt that does not exist
@ in that repository; the repository's actual LICENSE file is MIT, dated 2026,
@ and that is the grant relied on here.
@
@ WHY IT IS HERE. Nintendo's m4a resamples, volume-scales and reverbs every
@ voice DIRECTLY INTO the 8-bit DMA buffers, so quantisation error compounds
@ once per voice and the noise floor rises with the arrangement. This keeps a
@ 16-bit intermediate and quantises once, at the end.
@
@ WHAT IT DOES NOT TOUCH: the four legacy PSG/CGB channels. Those are hardware
@ oscillators driven by CgbSound writing REG_NR*, with no software mixing in
@ the path at all, so none of this reaches them.
@
@ LOCAL CHANGES, all in the configuration block, none in the code:
@   ENABLE_REVERB 0 -> 1  515 of the songs in midi.cfg declare reverb (-R50,
@                         one -R40), so building without it would change how
@                         the whole soundtrack sounds.
@   ENABLE_DMA            KEPT AT THE UPSTREAM DEFAULT OF 1. It was set to 0 on
@                         the first attempt, reasoning that upstream warns the
@                         DMA path can crash games not expecting the register
@                         writes and that this repo has its own DMA3 manager
@                         and HBlank effects. That was wrong twice over:
@                         ENABLE_DMA=0 selects an `.else` branch upstream does
@                         not ship or test, and ShinyDragonHunter's
@                         GameboySounds branch runs ENABLE_DMA=1 in a
@                         pokeemerald decomp, which disproves the conflict
@                         worry that motivated turning it off.
@   hq_buffer_ptr         left as SoundMainRAM_MixBuffer, defined at the foot
@                         of this file.
@
@ It replaces SoundMainRAM by SYMBOL: ours was thumb_func_start SoundMainRAM
@ called through `.word SoundMainRAM + 1`, and this is .thumb_func SoundMainRAM,
@ so the link resolves it with no pointer patching. The old implementation and
@ its two helpers were deleted from src/m4a_1.s because they shared one
@ .iwram.code section and --gc-sections could not reclaim them.
