...
The companion to ram_budget.py, and newly necessary: ROM was 79% and described
as "not the constraint" until the Gen 1 animated sprite roster took it to 91%.
It is a constraint now, so the question "what would we get back by cutting X"
needs an answer from data.
...
Run:  python rom_budget.py [--repo PATH] [--top N]
      ...
      python rom_budget.py --prefix gBwAnim                       # by symbol
Needs a linked build - pokeemerald.map comes from `make`.
"""
import argparse
import os
import re
...
#  what the bytes are.
CATEGORIES = [
    ('BW animation containers',  r'gBwAnim'),
