...
clean-assets:
	...
	find . \( -iname '*.1bpp' -o -iname '*.4bpp' -o -iname '*.8bpp' -o -iname '*.gbapal' -o -iname '*.lz' -o -iname '*.smol' -o -iname '*.fsmol' -o -iname '*.fastSmol' -o -iname '*.smolTM' -o -iname '*.rl' -o -iname '*.latfont' -o -iname '*.hwjpnfont' -o -iname '*.fwjpnfont' \) -exec rm {} +
...
# Generated by tools/rogue/emit_bw_anim.py. One explicit rule per animated
# species, because a frame container needs the frame size and the 4bpp file
# does not carry one.
include graphics/pokemon/bw_anim_rules.mk

# Generated by tools/rogue/emit_bw_trainer_anim.py, and separate from the
# species rules above because a different tool owns it: regenerating one roster
# must not rewrite the other's rules.
include graphics/trainers/bw_trainer_anim_rules.mk
